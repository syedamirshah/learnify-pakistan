...
from core.utils import send_account_notification_email  # ✅ Add this at the top
        ...
        if form.is_valid():
            ...
            for row in sheet.iter_rows(min_row=2, values_only=True):
                try:
                    ...
                    user.set_password(password)  # Set hashed password manually
                    user.save()
...
def admin_list_quizzes_view(request):
    ...
    return render(request, 'admin/core/list_quizzes.html', {
        ...
        'headers': headers,  # ✅ passed to template
    })
...
def list_backups(request):
    ...
    # ✅ If user clicked "Create Backup" button
    if request.method == "POST":
...
def restore_backup(request, filename):
    backup_dir = os.path.join(settings.MEDIA_ROOT, 'backups')  # ✅ FIXED
    filepath = os.path.join(backup_dir, filename)
...
def quiz_question_assignment_view(request):
    ...
    # Ô£ø√º√¨√• Apply grade filter if selected
    selected_grade_id = request.GET.get('grade_filter')
    if selected_grade_id:
        quizzes = quizzes.filter(grade_id=selected_grade_id)

    question_banks = QuestionBank.objects.all().order_by(Lower('title'))  # ‚Äö√∫√ñ Alphabetical sorting
    ...
    if request.method == 'POST':
        ...
        if quiz_id and bank_id and num_questions:
            try:
                ...
                return redirect('quiz-question-assignments')  # ‚Äö√∫√ñ works now
            except Exception as e:
                messages.error(request, f"‚Äö√π√• Error: {e}")
...
def bulk_delete_users(request):
    ...
    if not selected_ids:
        messages.warning(request, "⚠️ No users were selected for deletion.")
