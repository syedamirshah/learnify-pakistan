...
def normalize_text(value):
    """Normalize text for clean comparison (strip HTML, lower, collapse spaces)."""
...
def bulk_upload_scq(request, bank_id):
    ...
    if request.method == 'POST' and request.FILES.get('file'):
        ...
        messages.success(request, f"‚Äö√Ñ√∂‚àö√ë‚àö‚àÇ‚Äö√†√∂‚àö√´‚Äö√†√∂‚Äö√†√á‚Äö√Ñ√∂‚àö‚Ä†‚àö‚àÇ‚Äö√Ñ√∂‚àö‚Ä†¬¨¬•‚Äö√Ñ√∂‚àö‚Ä†‚àö‚àÇ‚Äö√†√∂¬¨¬± {uploaded} SCQ questions uploaded successfully. ‚Äö√Ñ√∂‚àö√ë‚àö‚àÇ‚Äö√†√∂‚àö√´‚Äö√†√∂‚Äö√†√á‚Äö√Ñ√∂‚àö‚Ä†‚àö‚àÇ‚âà√¨‚àö√ë‚Äö√Ñ√∂‚àö‚Ä†‚àö‚àÇ‚Äö√Ñ√∂‚àö√ë¬¨¬¢ {skipped} rows skipped.")
...
def bulk_upload_mcq(request, bank_id):
    ...
    if request.method == 'POST' and request.FILES.get('file'):
        ...
        messages.success(request, f"‚Äö√Ñ√∂‚àö√ë‚àö‚àÇ‚Äö√†√∂‚àö√´‚Äö√†√∂‚Äö√†√á‚Äö√Ñ√∂‚àö‚Ä†‚àö‚àÇ‚Äö√Ñ√∂‚àö‚Ä†¬¨¬•‚Äö√Ñ√∂‚àö‚Ä†‚àö‚àÇ‚Äö√†√∂¬¨¬± {uploaded} MCQ questions uploaded. ‚Äö√Ñ√∂‚àö√ë‚àö‚àÇ‚Äö√†√∂‚àö√´‚Äö√†√∂‚Äö√†√á‚Äö√Ñ√∂‚àö‚Ä†‚àö‚àÇ‚âà√¨‚àö√ë‚Äö√Ñ√∂‚àö‚Ä†‚àö‚àÇ‚Äö√Ñ√∂‚àö√ë¬¨¬¢ {skipped} rows skipped.")
        return redirect(f'/admin/core/questionbank/{bank.id}/change/')
...
def start_quiz(request, quiz_id):
    ...
    # ✅ Remove preview-mode slicing (handled per-bank now)
    # ❌ Removed:
    # if preview_mode:
...
def list_student_quiz_results(request):
    ...
    for attempt in attempts:
        ...
        results.append({
            ...
            'grade_letter': grade,
            'attempted_on': localtime(attempt.completed_at).strftime('%Y-%m-%d %H:%M')
        })
...
def get_quiz_result(request, attempt_id):
    ...
    # ‚Äö√Ñ√∂‚àö‚à´‚àö√± Check authorization
    ...
    for answer in answers:
        ...
        try:
            ...
            print(f"‚Äö√Ñ√∂‚àö‚àÇ‚Äö√Ñ‚Ä†‚àö√Æ‚Äö√†√®‚àö¬Æ Failed to find question with ID: {qid} for type: {qtype}")
            continue
    ...
    return Response({
        ...
        'completed_at': localtime(attempt.completed_at).strftime('%Y-%m-%d %H:%M'),
...
def submit_answer(request):
    ...
    if question_type == 'fib':
        if not isinstance(answer_data, dict) or any(str(v).strip() == '' for v in answer_data.values()):
            print("Incomplete FIB answer detected. All blanks must be filled.")
            return Response({'detail': 'All blanks must be filled for FIB questions.'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    try:
        ...
        print("‚Äö√Ñ√∂‚àö√ë‚àö‚àÇ‚Äö√†√∂‚àö√´‚Äö√†√∂‚Äö√†√á‚Äö√Ñ√∂‚àö‚Ä†‚àö‚àÇ‚âà√¨‚àö√ë‚Äö√Ñ√∂‚àö‚Ä†‚àö‚àÇ‚Äö√Ñ√∂‚àö√ë¬¨¬¢ Attempt not found or already finalized.")
    ...
    try:
        ...
        print("‚Äö√Ñ√∂‚àö√ë‚àö‚àÇ‚Äö√†√∂‚àö√´‚Äö√†√∂‚Äö√†√á‚Äö√Ñ√∂‚àö‚Ä†‚àö‚àÇ‚Äö√Ñ√∂‚àö‚Ä†¬¨¬•‚Äö√Ñ√∂‚àö‚Ä†‚àö‚àÇ‚Äö√†√∂¬¨¬± Answer saved successfully")
        return Response({'message': 'Answer submitted successfully.'}, status=status.HTTP_200_OK)

    except Exception as e:
        print("‚Äö√Ñ√∂‚àö√ë‚àö‚àÇ‚Äö√†√∂‚àö√´‚Äö√†√∂‚Äö√†√á‚Äö√Ñ√∂‚àö‚Ä†‚àö‚àÇ‚âà√¨‚àö√ë‚Äö√Ñ√∂‚àö‚Ä†‚àö‚àÇ‚Äö√Ñ√∂‚àö√ë¬¨¬¢ Exception while saving answer:", e)
        traceback.print_exc()
...
def finalize_quiz(request):
    ...
    # ‚Äö√Ñ√∂‚àö√ë‚àö‚àÇ‚Äö√†√∂‚àö√´‚Äö√†√∂‚Äö√†√á‚Äö√Ñ√∂‚àö‚Ä†‚àö‚àÇ‚Äö√Ñ√∂‚àö‚Ä†¬¨¬•‚Äö√Ñ√∂‚àö‚Ä†‚àö‚àÇ‚Äö√†√∂¬¨¬± This line was missing
...
def get_shining_stars(request):
    ...
    print("‚Äö√†√∂‚àö√Ü¬¨¬®¬¨¬£‚Äö√†√∂‚Äö√†√®‚Äö√Ñ√∂‚àö‚Ä†‚àö‚àÇ¬¨¬®‚Äö√†¬¥‚Äö√Ñ√∂‚àö‚Ä†‚àö‚àÇ¬¨¬®¬¨¬µ‚Äö√Ñ√∂‚àö√ë‚àö‚àÇ‚Äö√†√∂‚àö√´‚Äö√Ñ√∂‚àö√ë‚Äö√Ñ‚Ä†‚Äö√Ñ√∂‚àö‚Ä†‚àö‚àÇ‚Äö√†√∂‚àö√ú‚Äö√Ñ√∂‚àö√ë‚àö‚àÇ‚Äö√†√∂‚Äö√Ñ‚Ä†‚Äö√†√∂¬¨√Ü‚Äö√Ñ√∂‚àö‚Ä†‚àö‚àÇ¬¨¬®‚àö√ú Honor Roll Data:", json.dumps(honor_roll, indent=2))
...
def public_register_user(request):
    ...
    print(serializer.errors)  # Ô£ø√º√´√† Add this line temporarily
...
def renew_subscription(request):
    ...
    # ‚Äö√∫√ñ Save plan and receipt
    user.subscription_plan = plan
    user.fee_receipt = receipt
    user.renewal_requested = True

    # ‚Äö√∫√ñ Compute new expiry date (but don't activate yet)
    today = timezone.now().date()
    ...
    # ‚Äö√π√• DO NOT ACTIVATE ACCOUNT HERE
...
def teacher_student_list(request):
    ...
    # ‚Äö√∫√ñ Debug: See teacher info
    print(f"Ô£ø√º√´¬Æ‚Äö√Ñ√ßÔ£ø√º√®¬¥ Teacher Logged In: {user.username}")
    print(f"Ô£ø√º√®¬¥ School: {user.school_name} | Ô£ø√º√®√¥√î‚àè√® City: {user.city}")
